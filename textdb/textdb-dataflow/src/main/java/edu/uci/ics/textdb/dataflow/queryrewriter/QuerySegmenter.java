 ...
 *
 * Two algorithms implemented to perform different use-case for segmenting
 * query: Dynamic Programming and Brute Force Brute Force algorithm is not
 * deprecated in absence of better method to return all possible tokenizations
 *
 * Dynamic Programming algorithm to take input a query string and output a
 * single member array of strings as most-likely rewritten query A query is a
 * string that may comprise of multiple space separated strings called terms
 *
 * For example -> ["newyorkcity"] -> ["new york city"]
 *
 * Brute force algorithm to take input a query string and output an array of
 * strings as rewritten queries Algorithm tries to find a valid prefix of a term
 * and parses the rest of the term recursively After recursive call returns with
 * a set of parsed strings, prefix is appended in front of each string
 *
 * For example -> ["newyorkcity"] -> ["new york city", "newyorkcity"] Here, "new
 * york city" is a returned phrase because "new", "york" and "city" are valid
 * words in the word base.
 *
 */
public class QuerySegmenter {
     ...
     * delimiter For each term, rewrites(tokenizes) term to detect likely
     * missing spaces Concatenates all rewritten terms with singlespace
     * delimiter
     *
     * For example, if input query string is "newyork city", it would first
     * split it into "newyork" and "city" rewriteTermDP on "newyork" should
     * return "new york" rewriteTermDP on "city" should return "city" The two
     * rewritten strings are then concatenated in order by a singlespace to give
     * "new york city" The result returned as a single-member list is {"new york
     * city"}
     *
     * @return
     */
     ...
     * Programming algorithm approach used for tokenization of Chinese strings
    ...
    public static String rewriteTermDP(String term) {
        ...
        for (int endPosition = 1; endPosition < size; endPosition++) {
            for (int spanSize = 1; spanSize < RewriterConstants.MAX_WORD_LENGTH
                    && spanSize <= endPosition; spanSize++) {
                ...
                if (freq + scoreAtGap[endPosition - spanSize]
                        + RewriterConstants.TOKEN_LENGTH_PENALTY < scoreAtGap[endPosition]) {
                    scoreAtGap[endPosition] = freq + scoreAtGap[endPosition - spanSize]
                            + RewriterConstants.TOKEN_LENGTH_PENALTY;
     ...
     * term performs fuzzy tokenization to create corresponding lists of
     * rewritten terms Calls method rewriteTermBruteForce to perform
     * tokenization over single term Performs a cross product concatenation of
     * each rewritten term list and returns final String array Calls method
     * crossCatenate to perform required concatenation of multiple lists
     * 
     * @return
     */
     ...
     * tokenization of a term The original term may not be a valid dictionary
     * word, but be a term particular to the database The method rewriteBrute
     * only considers terms which are valid dictionary words In case original
     * term is not a dictionary word, it will not be added to queryList by
     * method rewriteBrute This wrapper ensures that if rewriteBrute does not
     * include the original term, it will still be included For example, for the
     * term "newyork", rewriteBrute will return the list <"new york"> But
     * "newyork" also needs to be included in the list to support particular
     * user queries This wrapper includes "newyork" in this list
     * 
     * @param term
     * @return
     */
     ...
     * term can be mapped to Assumes original term may have a tokenization
     * mistake due to missing multiple space delimiters Tries to find all
     * possible valid prefixes of original term as per word knowledge base For
     * each prefix, sends rest of string to a recursive call to get all possible
     * rewritten suffixes Attaches prefix term in front of each rewritten suffix
     * in the list Returns final term list
     * 
     * @param term
     * @return
     */
     ...
     * Function creates a combined list <l> containing all possible
     * K-combinations where each combination contains an entry from each of the
     * K lists combinations maintain order in which terms were parsed
     * 
     * @param allWordsList
     * @return
     */
